...
class CSPDarknet(nn.Cell):
    ...
    def construct(self, x):
        ...
        outputs = {}
        x = self.stem(x)
        outputs["stem"] = x     # 80*320*320, /2
        x = self.dark2(x)
        outputs["dark2"] = x    # 160*160*160， /4
        x = self.dark3(x)
        outputs["dark3"] = x    # 320*80*80， /8
        x = self.dark4(x)
        outputs["dark4"] = x    # 640*40*40， /16
        x = self.dark5(x)
        outputs["dark5"] = x    # 1280*20*20， /32
        return outputs["dark3"], outputs["dark4"], outputs["dark5"]
...
if __name__ == '__main__':
    ...
    # depthwise是否使用DWConv，False则使用BaseConv
    model = CSPDarknet(dep_mul=1.33, wid_mul=1.25, depthwise=False, act="silu")
    getrandtensor = ops.UniformReal(seed=2)
    input = getrandtensor((2, 3, 640, 640))

    out2, out1, out0 = model(input)
